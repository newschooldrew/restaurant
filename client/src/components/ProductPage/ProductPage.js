...
import ProductPageHeader from "../ProductPageHeader/ProductPageHeader.js";
...
function ProductPage({match}) {
      ...
      const {username,allMeals,cartItems,cart,meal} = state;
      const match_id = match.params.id;
      if(meal){
      const {_id,price,title} = meal;
      }
    
    useEffect(() => {
        fetchMealById(match_id,dispatch)
    ...
    const addItemToCart = meal =>{
        ...
        const {_id,title,price} = meal.meal;
        const id = meal.meal._id;
        console.log({_id,title,price})
        dispatch({type:"ADD_ITEM_TO_CART",payload:{id, title, price}})
    }
          ...
          <Container>
            <Row>
              ...
              <Col className="ml-auto mr-auto" md="6">
                ...
                <h5 className="category">{meal.description}</h5>
                <h2 className="main-price">${meal.price}</h2>
                <Button color="primary" size="small" variant="contained" onClick={() => addItemToCart({meal})}>
                    Add to Cart  
